...
unit_name="dlm-wonn-phase5-curve130k"
memory_high="${DLM_WONN_MEMORY_HIGH:-10G}"
memory_max="${DLM_WONN_MEMORY_MAX:-12G}"
pipeline="$repo_root/scripts/run_phase5_curve_extension_pipeline.sh"
...
command=(
    systemd-run --user
    "--unit=$unit_name"
    "--description=DLM-WONN WMT14 curve extension: ELF <=90K, WONN <=130K"
