...
class CloudPairConfigTest(unittest.TestCase):
    ...
    def test_50k_runtime_selection_uses_the_matching_five_checkpoints(self):
        config_root = REPO_ROOT / "src/configs/training_configs"
        payload = validate_pair_configs(
            config_root / "train_de-en-ELF-B-cloud-60k.yml",
            config_root / "train_de-en-WONN-L12K768T3-cloud-60k.yml",
        ...
        self.assertEqual(payload["terminal_optimizer_step"], 50000)
        self.assertEqual(payload["checkpoint_steps"], list(CHECKPOINT_STEPS[50000]))
    ...
    def test_pipeline_owns_training_and_four_gpu_evaluation(self):
        ...
        self.assertIn('target_steps="${DLM_WONN_TARGET_STEPS:-60000}"', pipeline)
...
class CloudPairAnalysisTest(unittest.TestCase):
    ...
    def test_50k_analysis_requires_both_runs_and_writes_one_composite_chart(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            common_config = {
                ...
                "save_optimizer_steps": "5000,10000,20000,40000,50000",
            }
            specs = (
            ...
            for label, run_dir, model in specs:
                ...
                training_rows = []
                for step in CHECKPOINT_STEPS[50000]:
            ...
            self.assertEqual(
                payload["comparison_contract"]["terminal_optimizer_step"], 50000,
            )
            self.assertEqual(len(payload["metrics_by_checkpoint"]), 10)
